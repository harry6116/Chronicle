...
# --- Chronicle Professional Enhancements (v9.3 Release) ---
# Added environment variable API key support for automation and professional deployment
...
def get_api_key():
    ...
    if not api_key:
        api_key = input("Please paste your Google API Key: ").strip()
    return api_key
...
def get_prompt(format_type, translate_mode, modernize_punctuation, condition_profiling, unit_conversion, image_descriptions):
    ...
    1. Document Metadata Header: At the very beginning of your output, you MUST provide the following tags on separate lines:
       {metadata_rules}
...
if __name__ == "__main__":
    try:
        ...
        # If main() is not defined explicitly, script will execute as originally structured
        pass
# ------------------------------------------------------------
